...
int main(int argc, char *argv[])
{
	...
	if (argc >= 2)
		...
		graph_path = "F:/Bio/20180103/newCommunity/";
	...
	while (!feof(fp))
	{
		...
		Communities cs;
		cs.load(graph_path + sfn);
		cs.removeBigComm(cs.comms, 10000);
		cs.save(graph_path + sfn + "10000");
		//cs.print();
		truth.push_back(cs);
	}
	fclose(fp);
	...
	fp = fopen((outdir + "f1_Truth_weighted.csv").c_str(), "w");
	...
	for (int i = 0; i < detected.size(); ++i)
	{
		...
		for (int truth_i = 0; truth_i < truth.size(); ++truth_i)
		{
			...
			double wf1 = Communities::wf1(truth[truth_i], detected[i]);
			fprintf(fp, ",%lf", wf1);
		}
		fprintf(fp, "\n");
